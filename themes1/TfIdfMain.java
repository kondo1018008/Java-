...
public class TfIdfMain {
	...
	public ArrayList<String> getFeatureWords(String st){
		HashMap<String,Double> comp= new HashMap<>();
		ArrayList<String> top5 = new ArrayList<>(5) ;
		for(Document d:this.docs) {
			if(d.getPath().equals(st)) {
				for(String s:d.getWords()) {
					comp.put(s,d.calcTF(s)-this.calcIDF(s));
				}
				
				List<Map.Entry<String,Double>> entries = new ArrayList<Map.Entry<String,Double>>(comp.entrySet());
